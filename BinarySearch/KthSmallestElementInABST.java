...
/**
 * Definition for a binary tree node with rank field.
 * rank represents how many nodes are in the left subtree.
 * public class TreeNode {
 *     int val;
 *     TreeNode left;
 *     TreeNode right;
...
public class KthSmallestElementInABST {
    public int kthSmallest(TreeNode root, int k) {
        int rank = root.rank;
        if(rank == k - 1) return root.val;
        else if(rank < k - 1) return kthSmallest(root.right, k - rank - 1);
